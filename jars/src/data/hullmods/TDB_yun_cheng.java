...
public class TDB_yun_cheng extends BaseHullMod implements HullModFleetEffect{
    private static final String id = "TDB_yun_cheng";
    ...
    public void advanceInCampaign(FleetMemberAPI member, float amount) {
        if (!Global.getSector().isPaused()) {
            ...
            if (fleet != null && fleet == Global.getSector().getPlayerFleet()) {
                if (fleet.getCargo().getQuantity(CargoAPI.CargoItemType.RESOURCES, "rare_ore")>=4) {
                    Day += Global.getSector().getClock().convertToDays(amount);
                    if (Day < 1) return;
                    Day -= 1;

                    fleet.getCargo().removeItems(CargoAPI.CargoItemType.RESOURCES, "rare_ore", 4f);
                    fleet.getCargo().addItems(CargoAPI.CargoItemType.RESOURCES, "rare_metals", 2f);
                }else {
                    if (fleet.getCargo().getQuantity(CargoAPI.CargoItemType.RESOURCES, "ore")>=10) {
                        Day += Global.getSector().getClock().convertToDays(amount);
                        if (Day < 1) return;
                        Day -= 1;

                        fleet.getCargo().removeItems(CargoAPI.CargoItemType.RESOURCES, "ore", 10f);
                        fleet.getCargo().addItems(CargoAPI.CargoItemType.RESOURCES, "metals", 5f);
                    }
                    if (fleet.getCargo().getQuantity(CargoAPI.CargoItemType.RESOURCES, "heavy_machinery")>=10) {
                        if (!fleet.isInHyperspace() && fleet.isInCurrentLocation()) {
                            Day += Global.getSector().getClock().convertToDays(amount);
                            if (Day < 2) return;
                            Day -= 1;

                            fleet.getCargo().addItems(CargoAPI.CargoItemType.RESOURCES, "rare_ore", 1f);
                        }
                    }
                }
            }
...
//        }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (Global.getSector().getPlayerFleet() != null) {
            CampaignFleetAPI fleet =  Global.getSector().getPlayerFleet().getFleetData().getFleet();
            if (fleet != null) {
                if (fleet.getCargo().getQuantity(CargoAPI.CargoItemType.RESOURCES, "rare_ore")>=4) {
                    text.addPara(txt("YC_18"), 0, Misc.getHighlightColor(), Misc.getPositiveHighlightColor(), txt("YC_21"));
                    text.addPara(txt("YC_19"), 4f);
                }else if(fleet.getCargo().getQuantity(CargoAPI.CargoItemType.RESOURCES, "ore")>=10){
                    text.addPara(txt("YC_18"), 0, Misc.getHighlightColor(), Misc.getPositiveHighlightColor(), txt("YC_22"));
                    text.addPara(txt("YC_19"), 4f);
                }else if (fleet.getCargo().getQuantity(CargoAPI.CargoItemType.RESOURCES, "heavy_machinery")>=10 && !fleet.isInHyperspace() && fleet.isInCurrentLocation()) {
                    text.addPara(txt("YC_18"), 0, Misc.getHighlightColor(), Misc.getPositiveHighlightColor(), txt("YC_24"));
                    text.addPara(txt("YC_19"), 4f);
                }else {
                    text.addPara(txt("YC_18"), 0, Misc.getHighlightColor(), Misc.getNegativeHighlightColor(), txt("YC_23"));
                    text.addPara(txt("YC_19"), 4f);
                }
            }else {
                text.addPara(txt("YC_18"), 0, Misc.getHighlightColor(), Misc.getNegativeHighlightColor(), txt("YC_23"));
