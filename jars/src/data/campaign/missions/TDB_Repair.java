			...
			for (FleetMemberAPI ship : members){
				...
				if (hullId.endsWith("_default_D")) {
					dialog.getTextPanel().addPara(
							ship.getShipName() + " — damaged variant ID: " + hullId,
							Misc.getNegativeHighlightColor()
					);
					String baseHullId = ship.getHullId().substring(0, ship.getHullId().length() - 10);
					dialog.getTextPanel().addPara(
							ship.getShipName() + " — calculated base ID: " + baseHullId,
							Misc.getNegativeHighlightColor()
					);
					...
					if (normalVariant != null) {
						...
						dialog.getTextPanel().addPara(
								ship.getShipName() + " — restored the damaged hull to its standard model.",
								Misc.getPositiveHighlightColor()
						);
					} else {
						dialog.getTextPanel().addPara(
								ship.getShipName() + " — correct base variant not found.",
								Misc.getNegativeHighlightColor()
						);
					}
				} else {
					dialog.getTextPanel().addPara(
							ship.getShipName() + " — no _default_D hull suffix; hull-spec restoration skipped.",
							Misc.getNegativeHighlightColor()
					);
				}
				...
				for (String tmp : BLOCKED_HULLMODS) {
					if (ship.getVariant().getHullMods().contains(tmp)) {
						DModManager.removeDMod(ship.getVariant(), tmp);
						String dModName = Global.getSettings().getHullModSpec(tmp).getDisplayName();
						String message = ship.getShipName() + " (" + ship.getHullSpec().getHullName() + "): repaired " + dModName + ".";
						dialog.getTextPanel().addPara(message, Misc.getTextColor(), Misc.getPositiveHighlightColor(), dModName);
					}
				}
