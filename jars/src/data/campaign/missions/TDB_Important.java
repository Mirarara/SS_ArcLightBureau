...
public class TDB_Important extends HubMissionWithSearch {
	...
	protected boolean create(MarketAPI createdAt, boolean barEvent) {
		...
		triggerFleetSetName("Invincible Fleet");
		//设置舰队所属任务，一般用不上
		triggerSetFleetMissionRef("$TDB_important_ref");
		triggerFleetSetPatrolActionText("Chirping");
		//令舰队在目标实体附近巡逻
	...
	public void addDescriptionForNonEndStage(TooltipMakerAPI info, float width, float height) {
		...
		if (currentStage == Stage.GO_TO_PROBE) {
			info.addPara("Recover the data package from the designated wreck field in " + system.getNameWithLowercaseTypeShort() + ".", opad);
		} else if (currentStage == Stage.RETURN_TO_CHALDEA) {
			info.addPara("Return to "+ getPerson().getMarket().getPrimaryEntity().getName() + " and speak with " + getPerson().getNameString() + " to receive your payment.", opad);
		}
	}
	...
	public boolean addNextStepText(TooltipMakerAPI info, Color tc, float pad) {
		...
		if (currentStage == Stage.GO_TO_PROBE) {
			if (system.isCurrentLocation()) {
				info.addPara("Mission objective completed", tc, pad);
			} else {
				info.addPara(getGoToSystemTextShort(system), tc, pad);
			}
			return true;
		} else if (currentStage == Stage.RETURN_TO_CHALDEA) {
			info.addPara("Return to "+ getPerson().getMarket().getPrimaryEntity().getName() +" and speak with " + getPerson().getNameString() + ".", tc, pad);
			return true;
		}
		return false;
	}
	...
	public void accept(InteractionDialogAPI dialog, Map<String, MemoryAPI> memoryMap) {
		...
		if (customFleet != null) {
			...
			//设置舰队行为：巡逻并拦截玩家
			customFleet.addAssignment(FleetAssignment.DEFEND_LOCATION, object, Float.MAX_VALUE, "Salvaging");
			customFleet.addAssignment(FleetAssignment.INTERCEPT, null, Float.MAX_VALUE, "Intercepting the player");
			customFleet.setName("Invincible Fleet");
	...
	private static class TDBImportantIntel extends BaseIntelPlugin{
		...
		public void createSmallDescription(TooltipMakerAPI info, float width, float height) {
			...
			info.addSectionHeading("Mission Objectives", Alignment.MID, pad);
			LabelAPI t = info.addPara("Travel to the %s system and salvage the mission objective", pad, Misc.getTextColor(), Misc.getHighlightColor(), system.getName());
			t.setHighlightColors(system.getStar().getSpec().getIconColor(), Misc.getHighlightColor());
			LabelAPI t1 = info.addPara("Return to the designated location on %s to deliver the mission item", pad, Misc.getTextColor(), Misc.getHighlightColor(), "Intersection");
